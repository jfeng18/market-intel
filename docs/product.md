...
- 不做只提升浏览效率、但不能进入复盘闭环的功能。
- 不做只给结论、但没有证据字段、覆盖边界和完成标准的功能。
- 不做需要私有账号、交易权限或云端个人数据同步才能成立的核心流程。
- 不做只能给人看、不能被 agent 通过稳定 JSON 复跑的关键链路。
...
成熟 App 很适合回答“这只股票现在发生了什么”。`market-intel` 更应该回答：

1. 上一次为什么关注这只股票。
2. 当时缺什么证据。
3. 今天的新行情、板块变化或持仓变化是否验证了上一轮假设。
4. 如果没有验证，下一步该读哪个结构化命令、补哪类证据、写哪段 journal。

这让系统越用越像个人研究账本，而不是每天重新开始的一次性诊股结果。
...
`dashboard` 是面向全 A 的默认第一屏。它必须先展示覆盖底座：`all-a` 当前是否仍是 seed，A 股基础清单是否接入，行业/概念/指数字段覆盖率如何，哪些覆盖缺口会影响当天结论。用户和 agent 应先看 `data.coverage_context`，并按 `coverage_review` 完成覆盖边界确认，再看市场热点和持仓压力。若 `top_data_quality_queue` 非空，`coverage_review` 的第一条只读命令应聚焦到 `pool quality <flag>`，而不是让用户在完整 coverage 里重新找样本。

全 A 迭代不能演变成“每个板块写一套特殊规则”。新增板块能力时应优先沉到通用字段：

- 行业、概念、指数成分、上市状态和基础证券属性。
- 研究状态、核心逻辑、关键证据和证伪风险。
- 持仓覆盖、重复暴露、缺口队列和单票复核命令。
- 板块强弱、候选标的、覆盖状态和 `done_when`。

只有当某个板块确实有跨股票通用、可验证且能进入 JSON contract 的专属字段时，才加入板块特化逻辑。
...
新增能力先过这一组问题，过不了就不做，或只作为外部数据输入：

1. 是否让全 A 覆盖边界更清楚，而不是制造“看起来很全”的错觉。
2. 是否能映射到用户持仓、观察池或研究证据缺口。
3. 是否能输出稳定 JSON 字段、下一步命令和 `done_when`。
4. 是否能进入 journal 留痕，并在下一次复盘中被验证或推翻。
5. 是否能保持本地私有数据边界，不要求把真实持仓、账号或本机路径写入公开资料。
6. 是否避免买卖建议、目标价、仓位建议和自动下单。

这组标准也回答“我们和股票 App 怎么竞争”：成熟 App 负责信息广度、实时性和交易入口；`market-intel` 负责把这些输入变成用户自己的复盘队列、证据账本和 agent 可接力流程。短期不追求资讯更快、指标更多、页面更像行情终端，而追求每天复盘更少遗漏、更好解释、更容易交接、更能回看。
