...
`pool universe --dry-run --json` 会导出 A 股基础清单补数 CSV 模板。模板按 `symbol/name/industry/concepts/index_membership/listing_status/source` 输出，默认覆盖当前所有缺行业、概念或指数成分的标的；可用 `--limit` 先抽样。填好后用 `import universe <patch.csv> --runtime --merge --dry-run --json` 复验，再正式 `--merge` 写入 runtime。
