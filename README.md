...
`pool universe --dry-run --json` 会把 `universe.enrichment_queue` 导出成 A 股基础清单补数 CSV 模板。模板按 `symbol/name/industry/concepts/index_membership/listing_status/source` 输出，只包含当前缺字段样本；填好后用 `import universe <patch.csv> --runtime --merge --dry-run --json` 复验，再正式 `--merge` 写入 runtime。
...
market-intel import universe data/runtime/a_share_universe_patch.csv --runtime --merge --dry-run --json
```
